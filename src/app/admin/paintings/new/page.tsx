  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrors({ submit: errorData.error || 'Failed to create painting' });
      }
    } catch (error) {
      console.error('Submit error:', error);
      setErrors({ submit: 'An error occurred while creating the painting' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-gray-900 border-b border-gray-800">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center space-x-4 py-6">
            ...
            <h1 className="text-3xl font-bold text-gold">Add New Painting</h1>
          </div>
        </div>
      </div>
